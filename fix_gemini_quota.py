    ...
    try:
        ...
        if result['success']:
            ...
            if '429' in str(result['error']):
                print("\n⚠️  QUOTA EXCEEDED - See solutions below")
...
def show_quota_solutions():
    ...
    print("\n🛠️  SOLUTIONS FOR QUOTA ISSUES")
    print("=" * 50)
    
    print("📊 **CURRENT STATUS**: Free Tier (50 requests/day)")
    print("\n🔄 **IMMEDIATE SOLUTIONS**:")
    ...
    print("\n🚀 **UPGRADE OPTIONS**:")
    print("• **Tier 1** (Billing enabled): 1,000-4,000 RPM, 1M-4M TPM")
    print("• **Tier 2** ($250+ spent): 2,000-20,000 RPM, 3M-10M TPM") 
    print("• **Tier 3** ($1000+ spent): 10,000-30,000 RPM, 8M-30M TPM")
...
def show_recent_usage():
    ...
    print("\n📈 Recent API Usage (Last 24 hours)")
    print("-" * 50)
    ...
    print(f"📊 Total requests in last 24h: {total_requests}")
    
    if recent_gens:
        print("\n🕒 Recent generations:")
        for gen in recent_gens[:5]:
            status_icon = "✅" if gen.status == 'completed' else "❌"
            print(f"   {status_icon} {gen.created_at.strftime('%H:%M:%S')} - {gen.title[:30]}...")
    
    if total_requests >= 45:
        print("\n⚠️  WARNING: Approaching daily limit (50 requests)")
    elif total_requests >= 50:
        print("\n🚫 QUOTA EXCEEDED: You've used your daily limit")
